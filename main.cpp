...
struct SimResult
{
	...
	Eigen::VectorXd tension_analytical;
	Eigen::VectorXd tmax_mean;
};
...
WorkingCurveForm
parse_form(const std::string& text)
{
	if (text == "Linear" || text == "linear")
		return WorkingCurveForm::Linear;
	if (text == "Quadratic" || text == "quadratic")
		return WorkingCurveForm::Quadratic;
	if (text == "Exponential" || text == "exponential")
		return WorkingCurveForm::Exponential;
	throw std::invalid_argument("Unknown working-curve form: " + text);
}

std::string
form_to_string(WorkingCurveForm form)
{
	switch (form) {
		case WorkingCurveForm::Linear:
			return "Linear";
	     ...
	     bool superimpose_fast)
{
	...
	if (!fairlead) {
		throw std::runtime_error("Failed to fetch fairlead point for " +
		                         c.input_file);
	}
	...
	for (unsigned int i = 0; i < nsteps; ++i) {
		...
		if (std::abs(dt_in - dt0) > 1e-12) {
			throw std::runtime_error("MoorDyn returned unexpected dt");
		}
		t = t_in;
	}
...
print_usage(const char* exe)
{
	std::cout
	    << "Usage:\n"
	    << "  " << exe
	    << " [--owc path] [--case all|linear|quadratic|exponential]"
	       " [--superimpose-fast 0|1]\n\n"
	    << "Defaults:\n"
	    << "  --owc input/owc.dat\n"
	    << "  output CSVs in each case input directory\n"
	    << "  --superimpose-fast 1\n"
	    << "  --case all\n";
}
...
{
	try {
		std::string owc_path = "input/owc.dat";
		std::string case_arg = "all";
		bool superimpose_fast = true;
		...
		for (int i = 1; i < argc; ++i) {
			...
			if (arg == "--superimpose-fast" && i + 1 < argc) {
				const std::string val = argv[++i];
				if (val == "1" || val == "true" || val == "TRUE") {
					superimpose_fast = true;
				}
				else if (val == "0" || val == "false" || val == "FALSE") {
					superimpose_fast = false;
				}
				else {
					throw std::invalid_argument(
					    "--superimpose-fast must be 0|1|true|false");
				}
				continue;
			}
		...
		const auto [owc_strains, owc_tensions] =
		    read_strain_tension_table(owc_path);

		std::vector<WcCase> cases;
		const std::vector<WcCase> defaults = {
			{ "Linear",
			  "input/linear_wc/line.txt",
			  WorkingCurveForm::Linear,
			  1.48657e-02 },
			{ "Quadratic",
			  "input/quadratic_wc/line.txt",
			  WorkingCurveForm::Quadratic,
			  1.50575e-02 },
			{ "Exponential",
			  "input/exp_wc/line.txt",
		...
		else if (case_arg == "linear") {
			cases.push_back(defaults[0]);
		}
		else if (case_arg == "quadratic") {
			cases.push_back(defaults[1]);
		}
		else if (case_arg == "exponential") {
			cases.push_back(defaults[2]);
		}
		else {
